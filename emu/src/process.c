...
/*
 * vfork synchronization: use a volatile atomic flag and
 * spin-wait.  We can't use pthread_cond (not signal-safe),
 * POSIX sem_t (macOS sem_init returns ENOSYS), or dispatch
 * semaphore (also doesn't wake from a signal handler
 * reliably).  A spin with usleep is the only portable
 * approach that works from inside a signal handler.
 */
#include <unistd.h>
...
proc_exit(emu_process_t *proc, int status)
{
	 ...
	 */
	__atomic_store_n(&proc->vfork_done, 1, __ATOMIC_SEQ_CST);
...
proc_fork(emu_process_t *parent)
{
	 ...
	 * the parent forever).
	 */
	while (!__atomic_load_n(&child->vfork_done, __ATOMIC_SEQ_CST)) usleep(1000);
    ...
    const char **envp)
{
	...
	if (ret != 0) {
		LOG_ERR("execve: cannot resolve %s", path);
		return (-ENOENT);
	}
